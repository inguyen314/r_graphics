...
Precip <- character(length(json_urls))
...
for (i in seq_along(json_urls)) {
  ...
  CurrentPoolStr[i] <- paste0(json_data[[1]]$pool_stage, " ft")
  CurrentTail[i] <- as.numeric(json_data[[1]]$tw_stage)
  CurrentTailStr[i] <- paste0(json_data[[1]]$tw_stage, " ft")
  Inflow[i] <- paste0(json_data[[1]]$inflow, " cfs")
  Outflow[i] <- paste0(json_data[[1]]$outflow_midnight, " cfs")
  SpillwayFlow[i] <- paste0(json_data[[1]]$outflow_evening, " cfs")
  ...
  Precip[i] <- as.numeric(json_data[[1]]$precip)
}
...
Res_data <- data.frame(
  ...
  Streambed = as.numeric(Streambed),
...
for (i in 1:length(Res_data$ReservoirName)){
  ...
  point_y <- c(Res_data$TopofDam[i]+10, Res_data$TopofFlood[i], Res_data$Streambed[i]+10)
  ...
  p1 <- ggplot() +  
    ...
    annotate("text", x= 750, y= as.numeric(Res_data$CurrentPool[i])-1.5, 
             ...
             color = "gray19", linewidth = 2.2, size = 3, fontface =1, hjust=1) + 
    ...
    # Draw Legend
    annotate("text", x= 1660, y=Res_data$TopofDam[i]+10, label= "Legend", 
