...
## version 6. add crest data with new JSON file. Add Precip and Generation
...
for (i in 1:length(Res_data$ReservoirName)) {
  ...
  scale_elev_y <- seq(round_any(Res_data$Streambed[i], 10),
                      round_any(Res_data$TopofDam[i] - 3, 10),
                      by = 10)
  ...
  point_x <- c(500,950,1015,950,350)
  point_y <- c(Res_data$TopofDam[i]+10, Res_data$TopofFlood[i], Res_data$Streambed[i]+10, Res_data$TopofDam[i]+0, Res_data$TopofDam[i]+20)
  point_label <- c("Inflow", "Spillway", "Outflow", "Generate", "Precipitation")
  point_label_short <- c("IN", "SPWY", "OUT", "GEN", "P")
  point_flow <- c(Res_data$Inflow[i], Res_data$SpillwayFlow[i], Res_data$Outflow[i], Res_data$Precip[i], Res_data$Precip[i])
  point_color <- c("steelblue2", "steelblue2", "steelblue4", "steelblue4", "steelblue4")
  point_DF <- data.frame(point_x, point_y, point_label, point_label_short,
                         point_flow, point_color, stringsAsFactors = F)
  # Add coordinates for the "Generate" arrow
  arrow1x <- c(1000, 1000, 1005, 1010)  # Add a new x-coordinate for the arrow tip
  arrow1y <- c(point_DF$point_y[3], point_DF$point_y[3] - 10, point_DF$point_y[3], point_DF$point_y[3])  # Update y-coordinates for the arrow
  ...
  p1 <- ggplot() +  
    ...
    scale_colour_manual(values = c("skyblue2", "steelblue4"), 
                        labels = c("Inflow", "Spillway", "Outflow", "Generate", "Precipitation")) +
    ...
    geom_segment(data = point_DF, aes(x = point_x[4] + 30, xend = point_x[4] + 100, y = point_y[4], yend = point_y[4]), 
                 linewidth = 2, color = "skyblue2") +
    annotate("segment", x = point_x[4] + 30, xend = point_x[4] + 100, y = point_y[4], 
             yend = point_y[4], linejoin = "round", 
             arrow = arrow(type = "closed", length = unit(0.03, "npc")), color = "steelblue4") +
    
    # Draw Precipitation Line Segment/Arrow pointing down and 1/3 shorter
    geom_segment(data = point_DF, aes(x = point_x[5], xend = point_x[5], y = point_y[5]-2, 
                                      yend = point_y[5] - 5), linewidth = 2, color = "skyblue2") +
    annotate("segment", x = point_x[5], xend = point_x[5], y = point_y[5] - 5, 
             yend = point_y[5] - 6, linejoin = "round", 
             arrow = arrow(type = "closed", length = unit(0.03, "npc")), color = "steelblue4") +
    ...
    geom_text(data = point_DF, mapping = aes(x=point_x, y=point_y, label=point_label_short), 
              linewidth = 2.5, size = 2.5, color = "white", fontface=2) + 
    ...
    annotate("text", x= 240, y= Res_data$Streambed[i]-1, 
             ...
             color = "red", linewidth = 2, fontface =1, hjust=1) + 
    annotate("text", x= 527.5, y= as.numeric(Res_data$GuideCurveElev[i])+2, 
             label= paste("Rule Curve", paste0("(", Res_data$GuideCurveElev[i], "')")),
             color = "lightskyblue4", linewidth = 2, size = 3, fontface =1, hjust=0.5) + 
    
    
    # Draw the Pool and Tail-water Labels
    annotate("text", x= 625, y= as.numeric(Res_data$CurrentPool[i])-1.5, 
             label= Res_data$CurrentPoolStr[i], 
             color = "gray19", linewidth = 3, fontface =1, hjust=2) + 
    annotate("text", x= 750, y= as.numeric(Res_data$CurrentPool[i])-1.5, 
             label= paste0("(24 Hr Change: ", Res_data$Change24[i], ")"), 
             color = "gray19", linewidth = 2.2, size = 3.5, fontface =1, hjust=1) + 
    
    annotate("text", x= 1798, y= Res_data$Streambed[i]+3.5, 
             label= Res_data$CurrentTailStr[i], color = "gray19", linewidth = 3, fontface =1, hjust=1) +
    
    
    # Draw Title/Additional Graphic Text
    annotate("text", x=1600, y= Res_data$TopofDam[i]+30, 
             label= "FLOOD CONTROL STATUS", color = "black", linewidth = 4.5, fontface =2, hjust=1) + 
    annotate("text", x=1600, y= Res_data$TopofDam[i]+24, 
             label= paste(Res_data$ReservoirName[i], "Lake, MVS"), 
             color = "gray19", linewidth = 3.5, fontface =1, hjust=1) + 
    annotate("text", x= 1600, y=Res_data$TopofDam[i]+19, label= date_time, 
             color = "gray19", linewidth = 3.5, fontface =1, hjust=1) + 
    
    
    # Draw Legend
    annotate("text", x= 1700, y=Res_data$TopofDam[i]+10, label= "Legend",
    ...
    geom_point(data = Res_data, mapping = aes(x=1480, y=TopofDam[i]-9.5), color="skyblue2", linewidth = 6) + 
    ...
    geom_point(data = Res_data, mapping = aes(x=1480, y=TopofDam[i]-16.5), color="skyblue2", linewidth = 6) + 
    annotate("text", x= 1480, y=Res_data$TopofDam[i]-16.5, label= "IN", 
             color = "white", linewidth = 1.5, fontface=2) + 
    annotate("text", x= 1520, y=Res_data$TopofDam[i]-16.5, label= "Inflow", 
             color = "gray19", linewidth = 2.5, hjust=0) + 
    
    geom_point(data = Res_data, mapping = aes(x=1480, y=TopofDam[i]-23.5), color="skyblue2", linewidth = 6) + 
    annotate("text", x= 1480, y=Res_data$TopofDam[i]-23.5, label= "SPWY", 
             color = "white", linewidth = 1.5, fontface=2) + 
    annotate("text", x= 1520, y=Res_data$TopofDam[i]-23.5, label= "Spillway", 
             color = "gray19", linewidth = 2.5, hjust=0) + 
    
    geom_point(data = Res_data, mapping = aes(x=1480, y=TopofDam[i]-30.5), color="steelblue4", linewidth = 6) + 
    annotate("text", x= 1480, y=Res_data$TopofDam[i]-30.5, label= "OUT", 
             color = "white", linewidth = 1.5, size = 3, fontface=2) + 
    annotate("text", x= 1520, y=Res_data$TopofDam[i]-30.5, label= "Outflow", 
             color = "gray19", linewidth = 2.5, hjust=0) + 
    
    geom_point(data = Res_data, mapping = aes(x=1480, y=TopofDam[i]-37.5), color="steelblue4", linewidth = 6) + 
