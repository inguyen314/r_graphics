...
required_packages <- c("ggplot2", "numform", "plyr", "dplyr", "lubridate", 
                       "rvest", "stringr", "stringi", "RColorBrewer", "jsonlite")
...
for (i in 1:length(Res_data$ReservoirName)) {
  ...
  p1 <- ggplot() +  
    ...
    annotate("text", x = 1720, y = Res_data$TopofDam[i] + 10, label = "Legend", color = "black", size = 3, fontface = 2, hjust = 2) +
    ...
    annotate("text", x = 1520, y = Res_data$TopofDam[i] + 2, label = "Current Lake Level", color = "gray19", size = 2.5, hjust = 0) +
    ...
    annotate("text", x = 1520, y = Res_data$TopofDam[i] - 3.5, label = "Tail Water", color = "gray19", size = 2.5, hjust = 0) +
