...
for (i in 1:length(Res_data$ReservoirName)) {
  ...
  if (Res_data$ReservoirName[i] == "Mark Twain Lk-Salt") {
    ...
    point_color <- c("skyblue2", "steelblue4", "steelblue4", "skyblue2")
  ...
  p1 <- ggplot() +  
    ...
    scale_colour_manual(values = c("skyblue2", "steelblue4", "steelblue4", "skyblue2", "skyblue2"), 
                        labels = c("Inflow", "Outflow", "Precipitation", "Generate", "Spillway")) +
    ...
    annotate("segment", x=point_DF$point_x[1], xend=point_DF$point_x[1], y=point_DF$point_y[1], 
             yend=point_DF$point_y[1]-16, linejoin="round", arrow=arrow(type="closed", length=unit(0.03, "npc")), color="skyblue2") +
