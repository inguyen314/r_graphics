...
ReservoirNames2 <- character(length(json_urls))
CurrentPool2 <- character(length(json_urls))
CurrentPoolStr2 <- character(length(json_urls))
CurrentTail2 <- character(length(json_urls))
CurrentTailStr2 <- character(length(json_urls))
Inflow2 <- character(length(json_urls))
Outflow2 <- character(length(json_urls))
SpillwayFlow2 <- character(length(json_urls))
Change242 <- character(length(json_urls))
GuideCurveElev2 <- character(length(json_urls))
...
for (i in seq_along(json_urls)) {
  ...
  ReservoirNames2[i] <- json_data[[1]]$project
  CurrentPool2[i] <- as.numeric(json_data[[1]]$pool_stage)
  CurrentPoolStr2[i] <- json_data[[1]]$pool_stage
  CurrentTail2[i] <- as.numeric(json_data[[1]]$tw_stage)
  CurrentTailStr2[i] <- json_data[[1]]$tw_stage
  Inflow2[i] <- as.numeric(json_data[[1]]$inflow)
  Outflow2[i] <- as.numeric(json_data[[1]]$outflow_midnight)
  SpillwayFlow2[i] <- as.numeric(json_data[[1]]$outflow_evening)
  Change242[i] <- as.numeric(json_data[[1]]$pool_24hr)
  GuideCurveElev2[i] <- as.numeric(json_data[[1]]$guide_curve)
  ...
  TopofDam[i] <- as.numeric(json_data[[1]]$top_of_con)
}
...
for (i in seq_along(json_urls)) {
  ...
  cat("ReservoirName2:", ReservoirNames2[i], "\n")
  cat("CurrentPool2:", CurrentPool2[i], "\n")
  cat("CurrentPoolStr2:", CurrentPoolStr2[i], "\n")
  cat("CurrentTail2:", CurrentTail2[i], "\n")
  cat("CurrentTailStr2:", CurrentTailStr2[i], "\n")
  cat("Inflow2:", Inflow2[i], "\n")
  cat("Outflow2:", Outflow2[i], "\n")
  cat("SpillwayFlow2:", SpillwayFlow2[i], "\n")
  cat("Change242:", Change242[i], "\n")
  cat("GuideCurveElev2:", GuideCurveElev2[i], "\n")
...
# Define Important reservoir extents (NOT IN USE)
...
class(Res_data2$Streambed)

Res_data2$Streambed <- as.numeric(Res_data2$Streambed)
Res_data2$BottomofFlood <- as.numeric(Res_data2$BottomofFlood)
Res_data2$TopofFlood <- as.numeric(Res_data2$TopofFlood)
Res_data2$TopofDam <- as.numeric(Res_data2$TopofDam)
...
for (i in 1:length(Res_data2$ReservoirName)){
  ...
  # Calculate Streambed - 10
  streambed_minus_10 <- Res_data2$Streambed[i] - 10
  top_of_dam_plus_30 <- Res_data2$TopofDam[i]+30
  
  # Check if Streambed - 10 is numeric and finite
  if (is.numeric(streambed_minus_10) && is.finite(streambed_minus_10)) {
    cat("Streambed - 10 is numeric and finite.\n")
  } else {
    cat("Streambed - 10 is not numeric and/or not finite.\n")
  }
  
  if (is.numeric(top_of_dam_plus_30) && is.finite(top_of_dam_plus_30)) {
    cat("Top of Dam + 30 is numeric and finite.\n")
  } else {
    cat("Top of Dam + 30 is not numeric and/or not finite.\n")
  }
  
  # Print values for troubleshooting
  cat("Streambed:", Res_data2$Streambed[i], "\n")
  cat("Top of Dam:", Res_data2$TopofDam[i], "\n")
  cat("Y-axis lower limit:", Res_data2$Streambed[i] - 10, "\n")
  cat("Y-axis upper limit:", Res_data2$TopofDam[i] + 30, "\n")
  ...
  p1 <- ggplot() +  
    ...
    # Print X-axis limits after ggplot() to check values
    cat("X-axis limits after ggplot():", range(resx), "\n")
