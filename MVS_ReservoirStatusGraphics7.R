...
#print(paste("json_data:", json_data))
...
for (project_id in names(json_data)) {
  ...
  # Extracting and appending information to respective lists
  # Appending information for each variable
...
for (i in 1:length(Res_data$ReservoirName)) {
  ...
  if (Res_data$ReservoirName[i] == "Carlyle Lk-Kaskaskia2") {
    # Define inflow/outflow arrows
    point_x <- c(500,1015,350,950)
    point_y <- c(Res_data$TopofDam[i]+10, Res_data$Streambed[i]+10, Res_data$TopofDam[i]+20, Res_data$TopofFlood[i])
    point_label <- c("Inflow", "Outflow", "Precipitation", "Spillway")
    point_label_short <- c("IN", "OUT", "P", "SPWY")
    point_flow <- c(Res_data$Inflow[i], Res_data$Outflow[i], Res_data$Precip[i], Res_data$SpillwayFlow[i])
    point_color <- c("steelblue2","steelblue4", "steelblue4",  "steelblue2")
    point_DF <- data.frame(point_x, point_y, point_label, point_label_short,
                           point_flow, point_color, stringsAsFactors = F)
    arrow1x <- c(1000, 1000, 1000, 1000)
    arrow1y <- c(point_DF$point_y[3], point_DF$point_y[3], point_DF$point_y[3], point_DF$point_y[3])
  } else  if (Res_data$ReservoirName[i] == "Mark Twain Lk-Salt") {
    # Define inflow/outflow arrows
    point_x <- c(500,1015,350,950)
    point_y <- c(Res_data$TopofDam[i]+10, Res_data$Streambed[i]+10, Res_data$TopofDam[i]+20, Res_data$TopofDam[i]+0)
    point_label <- c("Inflow", "Outflow", "Precipitation", "Generate")
    point_label_short <- c("IN", "OUT", "P", "GEN")
    point_flow <- c(Res_data$Inflow[i], Res_data$Outflow[i], Res_data$Precip[i], Res_data$Turbine[i])
    point_color <- c("steelblue2", "steelblue4", "steelblue4", "steelblue4")
    point_DF <- data.frame(point_x, point_y, point_label, point_label_short,
                           point_flow, point_color, stringsAsFactors = F)
    arrow1x <- c(1000, 1000, 1000, 1000)
    arrow1y <- c(point_DF$point_y[3], point_DF$point_y[3], point_DF$point_y[3], point_DF$point_y[3])
    ...
    # Define inflow/outflow arrows
    point_x <- c(500,1015,350)
    point_y <- c(Res_data$TopofDam[i]+10, Res_data$Streambed[i]+10, Res_data$TopofDam[i]+20)
    point_label <- c("Inflow", "Outflow", "Precipitation")
    point_label_short <- c("IN", "OUT", "P")
    point_flow <- c(Res_data$Inflow[i], Res_data$Outflow[i], Res_data$Precip[i])
    ...
    point_y <- c(Res_data$TopofDam[i]+10, Res_data$Streambed[i]+10, Res_data$TopofDam[i]+20, Res_data$TopofFlood[i], Res_data$TopofDam[i]+0)
    point_label <- c("Inflow", "Outflow", "Precipitation", "Spillway", "Generate")
    point_label_short <- c("IN", "OUT", "P", "SPWY", "GEN")
    point_flow <- c(Res_data$Inflow[i], Res_data$Outflow[i], Res_data$Precip[i], Res_data$SpillwayFlow[i], Res_data$Turbine[i])
  ...
  p1 <- ggplot() +  
    ...
    scale_colour_manual(values = c("skyblue2", "steelblue4", "steelblue4", "skyblue2", "skyblue2"), 
                        labels = c("Inflow", "Outflow", "Precipitation", "Spillway", "Generate")) +
    ...
    geom_segment(data = point_DF, aes(x=point_x[1], xend=point_x[1], y=point_y[1], yend=point_y[1]-10), linewidth=2, color="skyblue2") +
    annotate("segment", x=point_DF$point_x[1], xend=point_DF$point_x[1], y=point_DF$point_y[1], 
      yend=point_DF$point_y[1]-12, linejoin="round", arrow=arrow(type="closed", length=unit(0.03, "npc")), color="skyblue2") +
    
    
    # Draw OUTFLOW Line Segment/Arrow
    geom_segment(data = point_DF, aes(x=point_x[2]+30, xend=point_x[2]+120, y=point_y[2], yend=point_y[2]), linewidth=2, color="steelblue4") +
    ...
    geom_segment(data = point_DF, aes(x=point_x[3]+30, xend=point_x[3]+120, y=point_y[3], yend=point_y[3]), 
    ...
    geom_segment(data = point_DF, aes(x = point_x[4] + 30, xend = point_x[4] + 100, y = point_y[4], yend = point_y[4]), 
                 linewidth = 2, color = "skyblue2") +
    annotate("segment", x = point_x[4] + 30, xend = point_x[4] + 110, y = point_y[4], 
             yend = point_y[4], linejoin = "round", 
             arrow = arrow(type = "closed", length = unit(0.03, "npc")), color = "skyblue2") +
    
    
    # Draw GENERATE Line Segment/Arrow
    geom_segment(data = point_DF, aes(x = point_x[5], xend = point_x[5], y = point_y[5]-2, 
                                      yend = point_y[5] - 7), linewidth = 2, color = "skyblue2") +
    annotate("segment", x = point_x[5], xend = point_x[5], y = point_y[5] - 7, 
             yend = point_y[5] - 8, linejoin = "round", 
             arrow = arrow(type = "closed", length = unit(0.03, "npc")), color = "steelblue4") +
    ...
    geom_text(data = point_DF, mapping=aes(x=point_x[4]+50, y=point_y[4]+5, label=point_flow[4]),
              linewidth = 2.5, size = 2.5, color= "grey19", hjust=0)+
    geom_text(data = point_DF, mapping=aes(x=point_x[5]+50, y=point_y[5], label=point_flow[5]),
              linewidth = 2.5, size = 2.5, color= "grey19", hjust=0)+
    ...
    geom_segment(data = Res_data, aes(x=375, xend=375, y=Streambed[i]-5, yend=TopofDam[i]+5.5),
                 color = "grey", linewidth = 2) +
    geom_curve(data = Res_data, aes(x=375, xend=350, y=TopofDam[i]+5, yend=TopofDam[i]+7),
               curvature = 0.2, angle = 90, ncp=100,  linewidth = 2, color="grey")+
    geom_segment(data= Res_data, aes(x=351, xend=25, y=TopofDam[i]+7, yend=TopofDam[i]+7),
                 color = "grey", linewidth = 2) +
    ...
    geom_segment(data = scale_elev_DF, mapping = aes(x=scale_elev_xbegin, xend=scale_elev_xend, 
              y=scale_elev_y, yend=scale_elev_y), color = "grey", linewidth = 0.4) + 
    geom_text(data = scale_elev_DF, mapping = aes(x=scale_elev_xbegin-30, y=scale_elev_y, 
              label=scale_elev_y), linewidth = 2, size=2.5, color = "gray60") + 
    
    
    # Draw Reservoir Pertinent Elevation Line Segments
    geom_segment(data=Res_data, aes(y=Streambed[i], yend=Streambed[i], 
              x=250, xend=370), linewidth=0.5, linetype=8, color='red') + 
    geom_segment(data=Res_data, aes(y=BottomofFlood[i], yend=BottomofFlood[i], 
              x=250, xend=735), linewidth=0.5, linetype=8, color='red') + 
    geom_segment(data=Res_data, aes(y=TopofFlood[i], yend=TopofFlood[i], 
              x=250, xend=735), linewidth=0.5, linetype=8, color='red') + 
    geom_segment(data=Res_data, aes(y=TopofDam[i], yend=TopofDam[i], 
              x=250, xend=370), linewidth=0.5, linetype=8, color='red') + 
    geom_segment(data=Res_data, aes(y=GuideCurveElev[i], yend=GuideCurveElev[i],
              x=pool_water_positions$pool_x[1]+8, 
              xend=pool_water_positions$pool_x[3]+100), linewidth=0.5, linetype=8, color='lightskyblue4') +
    
    
    # Draw the Reservoir Pertinent Elevation Labels
    annotate("text", x=240, y=Res_data$TopofDam[i]-1, 
             label=wrapper(paste("Top of Dam", paste0(Res_data$TopofDam[i], "'")), width=10), 
             color="red", linewidth=2, size=3.0, fontface=1, hjust=1) + 
    annotate("text", x=240, y=Res_data$TopofFlood[i]+1, 
             label=wrapper(paste("Top of Flood", paste0(Res_data$TopofFlood[i], "'")), width=15),
             color="red", linewidth=2, size=3.0, fontface=1, hjust=1) + 
    annotate("text", x=240, y=Res_data$BottomofFlood[i]-1, 
             label=wrapper(paste("Bottom of Flood", paste0(Res_data$BottomofFlood[i], "'")), width=15), 
             color="red", linewidth = 2, size=3.0, fontface=1, hjust=1) + 
    annotate("text", x=240, y=Res_data$Streambed[i]-1, 
             label=wrapper(paste("Streambed", paste0(Res_data$Streambed[i], "'")), width=10),
             color="red", linewidth=2, size=3.0, fontface=1, hjust=1) + 
    annotate("text", x=900, y=as.numeric(Res_data$GuideCurveElev[i])+0, 
             label=paste("Rule Curve ", paste0("(", Res_data$GuideCurveElev[i], "')")),
             color="lightskyblue4", linewidth=2, size=2.5, fontface=1, hjust=0.5) + 
    ...
    annotate(
      "text",
      x = 450,
      y = as.numeric(Res_data$CurrentPool[i]) - 1,
      label = paste0(
        Res_data$CurrentPoolStr[i], "(", Res_data$Change24[i], ")\n",
        Res_data$CurrentPoolDateTimeStr[1]
      ),
      color = "gray19",
      size = 2.5,
      fontface = 1,
    ...
    annotate(
      "text", 
      x = 1795, 
      y = Res_data$Streambed[i]+3.5, 
      label = Res_data$CurrentTailStr[i], 
      color = "gray19", 
      size = 3.0, 
      fontface = 1, 
      hjust = 1
    ...
    annotate("text", 
      x = 1600, 
      y = Res_data$TopofDam[i]+30, 
      label = "FLOOD CONTROL STATUS", 
      color = "black", 
      linewidth = 4.5, 
      fontface = 2, 
      hjust = 1
    ...
    # Draw Legend
    annotate("text", x=1720, y=Res_data$TopofDam[i]+10, label="Legend",
             color="black", linewidth=3, fontface=2, hjust=2) + 
    
    geom_segment(data=Res_data, aes(y=TopofDam[i]+6, yend=TopofDam[i]+6, 
              x=1460, xend=1800), linewidth=1, color='grey') +
    geom_rect(data=Res_data, mapping=aes(xmin=1460, xmax=1500, ymin=TopofDam[i], ymax=TopofDam[i]+4),
              fill="lightskyblue1")+
    annotate("text", x=1520, y=Res_data$TopofDam[i]+2, label="Current Lake Level", 
              color="gray19", linewidth=2.5, hjust=0) + 
    
    geom_rect(data=Res_data, mapping=aes(xmin=1460, xmax=1500, ymin=TopofDam[i]-1.5, ymax=TopofDam[i]-5.5),
              fill="skyblue2")+
    annotate("text", x=1520, y=Res_data$TopofDam[i]-3.5, label="Tail Water", 
              color="gray19", linewidth=2.5, hjust=0) + 
    
    geom_point(data=Res_data, mapping=aes(x=1480, y=TopofDam[i]-9.5), color="skyblue2", size=6) + 
    annotate("text", x=1480, y=Res_data$TopofDam[i]-9.5, label= "P", 
             color="white", size=1.5, fontface=2) + 
    annotate("text", x=1520, y=Res_data$TopofDam[i]-9.5, label="Precipitation", 
             color="gray19", size=3, hjust=0) + 
    
    geom_point(data=Res_data, mapping=aes(x=1480, y=TopofDam[i]-16.5), color="skyblue2", size=6) + 
    annotate("text", x=1480, y=Res_data$TopofDam[i]-16.5, label="IN", 
             color="white", size=1.5, fontface=2) + 
    annotate("text", x=1520, y=Res_data$TopofDam[i]-16.5, label="Inflow", 
             color="gray19", size=3, hjust=0) + 
    
    geom_point(data=Res_data, mapping=aes(x=1480, y=TopofDam[i]-23.5), color="skyblue2", size=6) + 
    annotate("text", x=1480, y=Res_data$TopofDam[i]-23.5, label="SPWY", 
             color="white", size=1.5, fontface=2) + 
    annotate("text", x=1520, y=Res_data$TopofDam[i]-23.5, label="Spillway", 
             color="gray19", size=3, hjust=0) + 
    
    geom_point(data=Res_data, mapping=aes(x=1480, y=TopofDam[i]-30.5), color="steelblue4", size=6) + 
    annotate("text", x=1480, y=Res_data$TopofDam[i]-30.5, label="OUT", 
             color="white", size=1.5, fontface=2) + 
    annotate("text", x=1520, y=Res_data$TopofDam[i]-30.5, label="Outflow", 
             color="gray19", size=3, hjust=0) + 
    
    geom_point(data=Res_data, mapping=aes(x=1480, y=TopofDam[i]-37.5), color="steelblue4", size=6) + 
    annotate("text", x=1480, y=Res_data$TopofDam[i]-37.5, label="GEN", 
             color="white", size=1.5, fontface=2) + 
    annotate("text", x=1520, y=Res_data$TopofDam[i]-37.5, label="Generate", 
             color="gray19", size=3, hjust=0) +
